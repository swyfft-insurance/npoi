...
namespace TestCases.XSSF.UserModel.Charts
{
    public class TestXDDFChartAxisOrder
    {
        ...
        [Test]
        public void TestBarSeriesWithoutTitleWritesNoSeriesText()
        {
            using XSSFWorkbook wb = new XSSFWorkbook();
            ISheet sheet = new SheetBuilder(wb, plotData).Build();
            XSSFDrawing drawing = (XSSFDrawing)sheet.CreateDrawingPatriarch();
            ...
            var bar = chart.CreateData<string, double>(ChartTypes.BAR,
                chart.CreateCategoryAxis(AxisPosition.Bottom), chart.CreateValueAxis(AxisPosition.Left));
            bar.AddSeries(
                XDDFDataSourcesFactory.FromStringCellRange(sheet, CellRangeAddress.ValueOf("A1:E1")),
                XDDFDataSourcesFactory.FromNumericCellRange(sheet, CellRangeAddress.ValueOf("A2:E2")));
            chart.Plot(bar);

            using XSSFWorkbook readBack = XSSFTestDataSamples.WriteOutAndReadBack(wb);
            XmlDocument xml = ChartXml(SingleChart(readBack), out XmlNamespaceManager ns);
